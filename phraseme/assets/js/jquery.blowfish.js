/**
 * Blowfish by Alexandr Gorlov <a.gorlov@gmail.com>
 *
 * @see https://github.com/agorlov/javascript-blowfish
 */
(function($){
  ...
  Blowfish.prototype = {
    ...
    decryptECB: function(string) {
      ...
      for (var i = 0; i < blocks; i++) {
        var block = string.substr(i * 8, 8);
        if (block.length < 8) {
          throw new Error("decryptECB string byte length must be a multiple of 8");
        }
        var xL, xR, xLxR;
    ...
    decryptCBC: function(string, iv) {
      ...
      for (var i = 0; i < blocks; i++) {
        var block = string.substr(i * 8, 8);
        if (block.length < 8) {
          throw new Error("decryptCBC string byte length must be a multiple of 8");
        }
        var xL, xR, xLxR;
        xLxR = this.split64by32(block);
        xL = xLxR[0];
